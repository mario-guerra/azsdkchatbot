...
api_key="84b13cae422e417d8e4e8da1a6b4d1ad"
...
Prioritize the content in the prompt when answering questions.
...
async def chat(user_input: str, previous_input) -> Tuple[bool, str, str]:
    ...
    rust = r'^\s*\/(rust).*$'
    python = r'^\s*\/(python|Python).*$'
    java = r'^\s*\/(java|Java).*$'
    js = r'^\s*\/(javascript|js|Javacript|JavaScript|JS).*$'
    net = r'^\s*\/(\.net|net|\.NET|NET|csharp|C#).*$'
